...
public class AntBehavior : MonoBehaviour
{
    ...
    [SerializeField] float check_pheromone = 1f;
    private float check_countdown = 1f;
    ...
    void checkfood()
    {
        ...
        float detection_displacement = 3f;
        Physics2D.OverlapCircle(center, detection_displacement, tofood, to_food_pheromone_colliders); 
        ...
        Physics2D.OverlapCircle(center, detection_displacement, tohome, to_home_pheromone_colliders);  
    ...
    void directionDecision()
    {
        ...
        {
            if(to_home_pheromone_in_range.Count > 0)
            {
                Vector2 calculated_direction = calculate_pheromones(to_home_pheromone_in_range);
                desired_direction = calculated_direction; // this is either += or just =; i'm not sure which is better;
            }
    ...
    Vector2 calculate_pheromones(List<GameObject> usedlist)
    {
        ...
        for(int i = usedlist.Count - 1; i >= 0; i--)
        {
            ...
            else
            {
                ...
                float angle = Vector2.SignedAngle(normal,towards_pheromone);
        ...
        if(totaldecay != 0 && totalangle !=0)
        {
            ...
            direction = new Vector2(Mathf.Sin(totalangle * Mathf.Deg2Rad), Mathf.Cos(totalangle * Mathf.Deg2Rad));
            direction = direction.normalized;
        }
        direction = Quaternion.Euler(0, 0, totalangle) * (Vector2)transform.up;
        return direction;
    }
    void lay_pheromones(String type)
    {
        ...
        if(countdown <= 0)
        {
            ...
            GameObject gameObject = Instantiate(pheromone, transform.position, Quaternion.identity, pheromone_placeholder);
